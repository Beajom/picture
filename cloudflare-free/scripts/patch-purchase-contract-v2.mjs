...
// Purchase list: add a one-click purchase contract action beside PDF preview.
...
// Purchase detail modal: expose the same contract action there as well.
...
// Reuse the already-reviewed contract generator body from the first patch script.
...
if(changed){
  ...
  console.log("Purchase contract generator patched into index.html (v2)")
}else{
